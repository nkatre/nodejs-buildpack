...
then
  ...
  APPDYNAMICS_CONTROLLER_PORT=$(echo "${VCAP_SERVICES-}" | jq -r '.['\""$key"\"'][0] | .credentials | .port ')		 +  APPDYNAMICS_CONTROLLER_PORT=$(echo "${VCAP_SERVICES-}" | jq -r --arg key $key '[.[][] | select(.name | contains($key))][0] | .credentials | .port')
  APPDYNAMICS_AGENT_ACCOUNT_NAME=$(echo "${VCAP_SERVICES-}" | jq -r '.['\""$key"\"'][0] | .credentials | .["account-name"] ')		 +  APPDYNAMICS_AGENT_ACCOUNT_NAME=$(echo "${VCAP_SERVICES-}" | jq -r --arg key $key '[.[][] | select(.name | contains($key))][0] | .credentials | .["account-name"]')
  		  
  APPDYNAMICS_CONTROLLER_SSL_ENABLED=$(echo "${VCAP_SERVICES-}" | jq -r '.['\""$key"\"'][0] | .credentials | .["ssl-enabled"] ')		 +  APPDYNAMICS_CONTROLLER_SSL_ENABLED=$(echo "${VCAP_SERVICES-}" | jq -r --arg key $key '[.[][] | select(.name | contains($key))][0] | .credentials | .["ssl-enabled"]')
  APPDYNAMICS_AGENT_ACCOUNT_ACCESS_KEY=$(echo "${VCAP_SERVICES-}" | jq -r '.['\""$key"\"'][0] | .credentials | .["account-access-key"] ')		 +  APPDYNAMICS_AGENT_ACCOUNT_ACCESS_KEY=$(echo "${VCAP_SERVICES-}" | jq -r --arg key $key '[.[][] | select(.name | contains($key))][0] | .credentials | .["account-access-key"]')
